require "test_helper"
require "test_fakes_jsonapi"

class SerializerJsonapiTest < ActiveModel::TestCase
  ...
  def test_attributes
    user = User.new
    user_serializer = Jsonapi::DefaultUserSerializer.new(user, {})

    hash = user_serializer.as_json

    assert_equal({
      default_users: [{ first_name: "Jose", last_name: "Valim" }]
    }, hash)
  end

  def test_attributes_method
    user = User.new
    user_serializer = Jsonapi::UserSerializer.new(user, scope: {})

    hash = user_serializer.as_json

    assert_equal({
      users: [{ first_name: "Jose", last_name: "Valim", ok: true }]
    }, hash)
  end

  def test_attributes_method_specifying_keys
    user = User.new
    user_serializer = Jsonapi::UserAttributesWithKeySerializer.new(user, scope: {})

    hash = user_serializer.as_json

    assert_equal({
      user_attributes_with_keys: [{ f_name: "Jose", l_name: "Valim", ok: true }]
    }, hash)
  end

  def test_attributes_method_specifying_some_keys
    user = User.new
    user_serializer = Jsonapi::UserAttributesWithSomeKeySerializer.new(user, scope: {})

    hash = user_serializer.as_json

    assert_equal({
      user_attributes_with_some_keys: [{ first_name: "Jose", l_name: "Valim", ok: true }]
    }, hash)
  end

  def test_attributes_method_with_unsymbolizable_key
    user = User.new
    user_serializer = Jsonapi::UserAttributesWithUnsymbolizableKeySerializer.new(user, scope: {})

    hash = user_serializer.as_json

    assert_equal({
      user_attributes_with_unsymbolizable_keys: [{ first_name: "Jose", :"last-name" => "Valim", ok: true }]
    }, hash)
  end

  def test_attribute_method_with_name_as_serializer_prefix
    object = Jsonapi::SomeObject.new("something")
    object_serializer = Jsonapi::SomeSerializer.new(object, {})

    hash = object_serializer.as_json

    assert_equal({
      somes: [{ some: "something" }]
    }, hash)
  end

  def test_serializer_receives_scope
    user = User.new
    user_serializer = Jsonapi::UserSerializer.new(user, scope: { scope: true })
  ...
  def test_serializer_receives_url_options
    user = User.new
    user_serializer = Jsonapi::UserSerializer.new(user, url_options: { host: "test.local" })
    assert_equal({ host: "test.local" }, user_serializer.url_options)
  end

  def test_serializer_returns_empty_hash_without_url_options
    user = User.new
    user_serializer = Jsonapi::UserSerializer.new(user)
  ...
  def test_pretty_accessors
    user = User.new
    user.superuser = true
    user_serializer = Jsonapi::MyUserSerializer.new(user)
  ...
  def test_has_many
    ...
    post_serializer = Jsonapi::PostSerializer.new(post, scope: user)
  ...
  def test_conditionally_included_associations
    ...
    post_serializer = Jsonapi::PostWithConditionalCommentsSerializer.new(post, scope: user)
  ...
  def test_conditionally_included_associations_and_attributes
    ...
    post_serializer = Jsonapi::PostWithMultipleConditionalsSerializer.new(post, scope: user)
  ...
  def test_overridden_associations
    author_serializer = Class.new(ActiveModel::SerializerJsonapi) do
      self.jsonapi_format = true
      attributes :first_name
    end

    blog_serializer = Class.new(ActiveModel::SerializerJsonapi) do
      self.jsonapi_format = true
    ...
    #With no classname, jsonapiformat has no root, but returns [] anyway
    assert_equal([{
  ...
  def post_serializer
    Class.new(ActiveModel::SerializerJsonapi) do
      self.jsonapi_format = true
      attributes :title, :body
      has_many :comments, serializer: Jsonapi::CommentSerializer
      has_one :author, serializer: Jsonapi::DefaultUserSerializer
    end
  end
  ...
  def test_associations_with_nil_association
    ...
    serializer = Class.new(Jsonapi::BlogSerializer) do
      root :blog
    end
  ...
  #   another_serializer = Class.new(Jsonapi::BlogSerializer) do
  #     self.root = false
  #   end
  ...
  def test_true_root_should_overwrite_jsonapi
    blog = Blog.new
    ...
    }, Jsonapi::BlogWithRootSerializer.new(blog).as_json)
  end
  ...
  #   post_serializer = Class.new(ActiveModel::SerializerJsonapi) do
  #     attributes :title
  #     has_many :comments
  #     embed :ids
  ...
  #   comment_serializer = Class.new(ActiveModel::SerializerJsonapi) do
  #     def id
  #       "OMG"
  #     end
  #   end
  ...
  def test_sets_can_be_serialized
    ...
    serializer = set.active_model_serializer.new set, each_serializer: Jsonapi::CustomPostSerializer
  ...
  def test_associations_with_as
    ...
    custom_blog = Jsonapi::CustomBlog.new
    custom_blog.public_posts = posts
    custom_blog.public_user = user

    serializer = Jsonapi::CustomBlogSerializer.new(custom_blog, scope: { scope: true })
  ...
  #   implicit_serializer = Class.new(ActiveModel::SerializerJsonapi) do
  #     self.jsonapi_format = true
  #     root :custom_blog
  #     const_set(:UserJsonapiSerializer, UserJsonapiSerializer)
  #     const_set(:PostJsonapiSerializer, PostJsonapiSerializer)
  ...
  #   posts = [
  #     PostJsonapi.new(title: 'First Post', body: 'text', comments: []),
  #     PostJsonapi.new(title: 'Second Post', body: 'text', comments: [])
  #   ]
  #   user = UserJsonapi.new
  ...
  def test_attribute_key
    serializer_class = Class.new(ActiveModel::SerializerJsonapi) do
      self.jsonapi_format = true
      root :user
  ...
  def test_schema
    ...
    serializer = Class.new(ActiveModel::SerializerJsonapi) do
  ...
  def test_embed_ids_include_true_with_root
    ...
    serializer_class.class_eval do
      ...
      has_one :author, serializer: Jsonapi::DefaultUserSerializer, key: :author_id, root: :author
    end
  ...
  def test_embed_with_include_inserts_at_root
    tag_serializer = Class.new(ActiveModel::SerializerJsonapi) do
      attributes :id, :name
    end
  ...
  #   email_serializer = Class.new(ActiveModel::SerializerJsonapi) do
  #     self.jsonapi_format = true
  ...
  #   attachment_serializer = Class.new(ActiveModel::SerializerJsonapi) do
  #     self.jsonapi_format = true
  #     attributes :name, :url
  ...
  #   orange_serializer = Class.new(ActiveModel::SerializerJsonapi) do
  #     embed :ids, include: true
  ...
  def test_raises_an_error_when_a_child_serializer_includes_associations_when_the_source_doesnt
    ...
    fruit_serializer = Class.new(ActiveModel::SerializerJsonapi) do
      self.jsonapi_format = true
      embed :ids, include: true
    ...
    smoothie_serializer = Class.new(ActiveModel::SerializerJsonapi) do
      self.jsonapi_format = true
      root false
      embed :ids, include: true
  ...
  def test_only_option_filters_attributes_and_associations
    ...
    post_serializer = Jsonapi::PostSerializer.new(post, only: :title)
  ...
  def test_except_option_filters_attributes_and_associations
    ...
    post_serializer = Jsonapi::PostSerializer.new(post, except: [:body, :comments])
  ...
  def test_only_option_takes_precedence_over_custom_defined_include_methods
    ...
    post_serializer = Jsonapi::PostWithMultipleConditionalsSerializer.new(post, scope: user, only: :title)
